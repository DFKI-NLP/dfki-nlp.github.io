...
def calculate_OS_IDF(collection):
    ...
    tokenized_documents = [nlp(doc.lower()) for doc in collection]  # Ensure lowercase
    ...
    # Adjust the IDF calculation for sensitive information
    for term, df in document_frequencies.items():
        if re.match(r'\bhttps?://\S+|www\.\S+|\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b|\b[A-Z][a-z]*\b|\b(?:Street|City|Road|Avenue|Boulevard|Town|Village|County)\b', term):
    ...
    for doc in noun_terms_documents:
        ...
        if doc_idf_values:
            avg_idf = round(sum(doc_idf_values) / len(doc_idf_values), 2)
...
def preprocess_document(doc):
    # Use spaCy's NER to detect names (entities of type PERSON)
    spacy_doc = nlp(doc)
    names = [ent.text for ent in spacy_doc.ents if ent.label_ == "PERSON"]

    # Capture sensitive information
    urls = re.findall(r'\bhttps?://\S+|www\.\S+', doc)
    emails = re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', doc)
    locations = [ent.text for ent in spacy_doc.ents if
                 ent.label_ in {"GPE", "LOC", "FAC"}]  # Detect locations using NER
    ...
    # Append the sensitive information back to the document to ensure they are included in the analysis
    doc = doc.strip() + ' ' + ' '.join(urls + emails + names + locations)
    return doc.strip()
